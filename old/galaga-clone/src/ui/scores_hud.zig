...
pub const ScoresHud = struct {
    ...
    pub fn draw(self: *const @This(), ctx: GameContext) void {
        ...
        if (ctx.game_state.active and ctx.game_state.current_player == 1 and self.flash_on) {
            ...
            ctx.renderer.drawText(score_text, pos_1up_score, text_grid.font_size, Color.white, arcade_font);
        }

        const hs_label = "HIGH SCORE";
        const hs_label_pos = text_grid.getCenteredPosition(hs_label, 0);
        ctx.renderer.drawText(hs_label, hs_label_pos, text_grid.font_size, Color.red, arcade_font);

        const hs = "20000";
        const hs_pos = text_grid.getCenteredPosition(hs, 1);
        ctx.renderer.drawText(hs, hs_pos, text_grid.font_size, Color.white, arcade_font);

        const label_2up = "   2UP ";
        const pos_label_2up = text_grid.getRightAlignedPosition(label_2up, 0);
        ctx.renderer.drawText(label_2up, pos_label_2up, text_grid.font_size, Color.red, arcade_font);
        ...
        if (ctx.game_state.active and ctx.game_state.current_player == 2 and self.flash_on) {
            ...
            ctx.renderer.drawText(score_text, pos_label_2up_score, text_grid.font_size, Color.white, arcade_font);
        }
        ...
        if (!ctx.game_state.active) {
            ...
            ctx.renderer.drawText(credits, pos_credits, text_grid.font_size, Color.white, arcade_font);
        }
    }
};
